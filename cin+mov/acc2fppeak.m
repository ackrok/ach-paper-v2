...
% Plot accelerationaligned to peaks (phase 0) 
% UPDATED: 17-May-2022
...
for x = 1:length(beh)
    ...
    %% Align ACCELERATION to peaks
    [fp2ph{y,1},staTime] = getSTA(acc, peakIdx./Fs, Fs, [-1 1]); % extract ACCELERATION values matching this phase
    for c = 1:2
        ii_c = peakIdx(ismember(peakIdx, idx_cell{c}));  % include index for this behavioral state
        fp2ph_state{y,c} = getSTA(acc, ii_c./Fs, Fs, [-1 1]); % extract ACCELERATION values for this behavioral state
    end
    c = 3;
    acc_rand = acc(randperm(length(acc),length(acc)));
    fp2ph_state{y,c} = getSTA(acc_rand, ii_c./Fs, Fs, [-1 1]); % extract ACCELERATION values for these random indices
...
for z = 1:2
    shadederrbar( staTime, nanmean(fp2ph_an{y,z},2), SEM(fp2ph_an{y,z},2), clr{z});
end
a = nanmean(fp2ph_prc_an{2} - fp2ph_prc_an{1},2);
b = nanmean(fp2ph_prc_an{3} - fp2ph_prc_an{2},2);
shadederrbar(staTime, nanmean(fp2ph_prc_an{2},2), nanmean(fp2ph_prc_an{1},2), 'k');

ylabel('Acceleration (cm/s^2)'); ylim([-0.1 0.15]);
xlabel(sprintf('Time to %s peak (s)',lbl)); xlim([-1 1]); xticks([-1:0.5:1]);
title(sprintf('Acceleration @ %s peak',lbl)); axis('square');
