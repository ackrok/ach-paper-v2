...
shadederrbar(flog(1:ds:end), nanmean(sub_1((1:ds:end),:),2), SEM(sub_1((1:ds:end),:),2), 'r');

% shadederrbar(flog(1:ds:end), nanmean(norm_dual_dls_ach((1:ds:end),:),2), SEM(norm_dual_dls_ach((1:ds:end),:),2), 'g');
% shadederrbar(flog(1:ds:end), nanmean(norm_dual_dms_ach((1:ds:end),:),2), SEM(norm_dual_dms_ach((1:ds:end),:),2), 'k');
% legend({'0.5Hz','4Hz','ACh imm','','ACh mov'})
...
% shadederrbar(flog(1:ds:end), nanmean(sub_gfp((1:ds:end),[1:6]),2), SEM(sub_gfp((1:ds:end),[1:6]),2), 'g'); 
% shadederrbar(flog(1:ds:end), nanmean(sub_gfp((1:ds:end),[7:12]),2), SEM(sub_gfp((1:ds:end),[7:12]),2), 'k'); 
% legend({'','0.5Hz','4Hz','DA imm','','DA mov'})
shadederrbar(flog(1:ds:end), nanmean(sub_gfp((1:ds:end),:),2), SEM(sub_gfp((1:ds:end),:),2), 'r'); 

xlabel('Frequency'); 
xlim([-1 flog(f == 50)]); xticks([-2:2]); xticklabels({'0.01','0.1','1','10','100'});
title('FFT subtracting'); ylabel('Power (a.u.)');
axis('square');
movegui(gcf,'center');
