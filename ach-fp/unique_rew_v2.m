...
for x = 1:length(beh); y = 1;
    ...
    while prc_captured(x) < 0.9 % Adjust threshold parameters until achieve 90%
        if counter >= 1; thres_adj = thres_adj + 0.2; end
        ...
        if thres_adj > -1; thres_adj = -1; break; end
    end
...
ylabel('ACh3.0 (%dF/F)'); ylim([-6 4]); yticks([-6:2:4]);
axis('square')
...
ylabel('magnitude (%dF/F)'); ylim([2 7]); yticks([2:7]);
