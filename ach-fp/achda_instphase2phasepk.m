...
for x = 1:length(beh)
    ...
    for z = 1:3 
        ph_ach{x,z} = rad2deg(fp_phase(fp_peak{1,z},1));
        ph_da{x,z} = rad2deg(fp_phase(fp_peak{1,z},2)); end
end
...
h(z).Title = 'IMM only: DA inst phase @ ACh peak';
h(z).XLabel = 'phase';
h(z).Colormap = jet; h(z).GridVisible = 'off'; % h(z).ColorLimits = [0 1];

subplot(1,3,2); hold on
shadederrbar(edges(2:end)-5, nanmean(phase_cell{1,1},2), SEM(phase_cell{1,1},2), 'g');
yyaxis right; shadederrbar(edges(2:end)-5, nanmean(phase_cell{2,1},2), SEM(phase_cell{2,1},2), 'm');
legend({'ACh','','DA'});
xlabel('phase'); xlim([-180 180]); xticks([-180:90:180]); 
title('DA inst phase @ ACh peak'); axis('square');

subplot(1,3,3); hold on
shadederrbar(edges(2:end)-5, nanmean(phase_cell{2,1},2), SEM(phase_cell{2,1},2), 'r');
shadederrbar(edges(2:end)-5, nanmean(phase_cell{2,2},2), SEM(phase_cell{2,2},2), 'g');
shadederrbar(edges(2:end)-5, nanmean(phase_cell{2,3},2), SEM(phase_cell{2,3},2), 'b');
legend({'imm','','mov','','rew'});
xlabel('phase'); xlim([-180 180]); xticks([-180:90:180]); 
title('DA inst phase @ ACh peak'); axis('square');