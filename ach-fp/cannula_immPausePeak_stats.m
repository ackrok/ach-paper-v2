...
r = [1 2]; a = dur_mat(:,r).*(1000/Fs);
errorbar(abs(nanmean(a)'),abs(SEM(a,1)'),'.k','MarkerSize',20);
plot([1.15; 1.85].*ones(2,nAn),abs(a'),'.:k','MarkerSize',20);
xlim([0.5 2.5]); xticks([1 2]); xticklabels({'aCSF','D1R/D2R antag'});
ylabel(sprintf('%s Duration (ms)',lbl)); ylim([-20 400]); yticks([0:100:500]);
[~, p] = ttest(a(:,1),a(:,2));
...
switch z; case 1; ylim([3 5]); yticks([3:5]); case 2; ylim([-0.2 15]); yticks([0:5:15]); end
[~, p] = ttest(a(:,1),a(:,2));
