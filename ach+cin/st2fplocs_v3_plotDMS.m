...
plot([0 0],[-0.2 0.2],'--k');
a = shuff50_rest; a = a - nanmean(a,1);
shadederrbar(time, movmean(nanmean(a,2),sm), movmean(nanmean(shuff5_rest,2),sm), 'k');
...
for y = 1:size(alignDelta_rest,2)
    ...
    b = shuff95_rest(:,y); b = b - nanmean(shuff50_rest(:,y));
    c = shuff50_rest(:,y) - (shuff95_rest(:,y) - shuff50_rest(:,y)); c = c - nanmean(shuff50_rest(:,y)); % mat(x).shuff5_rest(:,y);
