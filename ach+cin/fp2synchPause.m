...
for u = 1:length(uni)
    ...
    for x = 1:length(ii)
        ...
        st = [sub_uni(x).pause.Windows(:,1) + (sub_uni(x).pause.Windows(:,2)-sub_uni(x).pause.Windows(:,1))]; % Extract PAUSE times of this unit
        st_other = cell(length(jj),1);
        for y = 1:length(jj)
            st_other{y} = sub_uni(jj(y)).pause.Windows(:,1) + (sub_uni(jj(y)).pause.Windows(:,2)-sub_uni(jj(y)).pause.Windows(:,1)); % Extract PAUSE times of all other units from this recording
        end
        %% PETH
        peth = getClusterPETH(st_other, st, bin, window); % PETH: spike times aligned to PAUSE times
        %%
        prob = []; 
        t_0 = 1; cts0 = []; cts = {};
        ...
        [sta_fp, sta_time] = getSTA(fp, st, Fs, [-2, 1]);
...
for x = 1:length(mat)
    ...
    prc5 = []; prc50 = []; prc95 = [];
    for y = 1:nShuff
        tmp2 = getSTA(fp, shuffSt{y}, Fs, [-2, 1]); % align photometry to shuffled spike times
...
%% (3) 0/N IMMOBILITY
sta_zero = cell(length(mat),1);
sta_zero_imm = [];
sta_shuff = []; sta_shuff95 = [];
for x = 1:length(mat)
    cts0 = mat(x).cts0; % extract counts
    cts0(cts0 > 1) = 1; 
    cts0 = sum(cts0, 1); % sum across units
    i0 = find(cts0 == 0); i0 = i0'; % index of PAUSE coherence
    i0_imm = mat(x).idx_imm((ismember(mat(x).idx_imm, i0))); % index of zero coherence among units that occur during immobility
    pull_sta = mat(x).sta;
    pull_sta = pull_sta - nanmean(pull_sta([find(sta_time == -2):find(sta_time == -1)-1],:)); % subtract baseline [-2 -1]
    pull_sta = pull_sta([find(sta_time == -1):find(sta_time == 1)],:); % extract STA matrix for window [-1 1];
    sta_zero{x} = pull_sta(:,i0_imm);
    sta_zero_imm(:,x) = nanmean(sta_zero{x,1},2);
    pull_shuff = mat(x).prc_shuff([find(sta_time == -1):find(sta_time == 1)],:); % extract shuffled STA matrix for window [-1 1];
    sta_shuff(:,x) = pull_shuff(:,2);
    sta_shuff95(:,x) = pull_shuff(:,3) - pull_shuff(:,2);
end
sta_time1 = sta_time([find(sta_time == -1):find(sta_time == 1)]);

%% (3b) Plot STA IMM for each recording - IMMOBILITY
figure; plm = floor(sqrt(length(mat))); pln = ceil(length(mat)/plm);
for x = 1:length(mat)
    if isempty(sta_zero{x}); continue; end
    sp(x) = subplot(plm, pln, x);
    shadederrbar(sta_time, mat(x).prc_shuff(:,2), mat(x).prc_shuff(:,1) - mat(x).prc_shuff(:,2),'k');
    shadederrbar(sta_time1, nanmean(sta_zero{x},2), SEM(sta_zero{x},2),'r');
    xlim([-1 1]);
